...
<div class="container">
    ...
    <form method="POST" action="{{ route('demand::save') }}">
        ...
        <div class="form-group">
            <label>会社名</label><span class="label label-danger">必須</span>
             <select name="company_id">
                 ...
                 <option value="{{$companyName->company_id}}" @if (!empty($data)) @if ($data->company_id == $companyName->company_id ) selected @endif @endif>{{$companyName->company}}</option>
                 @endforeach
             </select>
        ...
        <div class="form-group">
            ...
            <textarea name="business" row="4" cols="40" placeholder="商談内容を入力してください">@if (!empty($data)){{$data->business}}@endif
            </textarea>      
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <select name="status">
                <option value="0" @if (!empty($data)) @if ($data->status == 0 ) selected @endif @endif>完了</option>
                <option value="1" @if (!empty($data)) @if ($data->status == 1 ) selected @endif @endif>進行中</option>
                <option value="2" @if (!empty($data)) @if ($data->status == 2 ) selected @endif @endif>未受注</option>
                <option value="3" @if (!empty($data)) @if ($data->status == 3 ) selected @endif @endif>保留</option>
                <option value="4" @if (!empty($data)) @if ($data->status == 4 ) selected @endif @endif>破棄</option>
            </select>
        </div> 
